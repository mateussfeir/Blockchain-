...
def valid_proof(transactions, last_hash, proof):
    ...
    # return guess_hash == real_hashed_block --  It would be the real hash
...
def proof_of_work():
    ...
    while not valid_proof(open_transactions, last_hash, proof):
        proof += 1
        time.sleep(0.0025) # It represents the same as 200 hashs/s, with my 5 GPU 1660 s I had computacional power enought to run 157 Mhash/s (on ETH hash)
    return proof
...
def load_data():
    ...
    except IOError: # That's an error when we can not access the file.  
        genesis_block = {
            'previous_hash': '',
            'index': 0,
            'transactions': [],
...
# QUESTION: Why do we need to use sum() on the amount_sent and amount_receveid?
...
real_hashed_block = [] # Just in case that we want to use the real hash mode.

def mine_block():
    hashed_block = hash_block(blockchain[-1])
    real_hashed_block.append(hashed_block) # Just in case that we want to use the real hash mode.
    print('hashed block: \n{}\n'.format(hashed_block))
    proof = proof_of_work()
    print('\nThe miners have tried {} different values to find the correct answer:\n'.format(proof))
    mining_reward_tx = OrderedDict([('sender', 'Mining reward'),('recipient', owner,),('amount', Mining_reward)])
    copied_open_transactions = open_transactions[:]
    copied_open_transactions.append(mining_reward_tx)
    block = {
